...
class ActionAdapter:
    ...
    def vectorize_action(self, action: dict[str, Any]) -> list[float]:
        ...
        vector = [float(move[0]), float(move[1]), use_push]
        if self.config.include_push_direction:
            vector.extend([float(push[0]), float(push[1])])
        return vector
    ...
    def action_from_vector(self, vector: list[float]) -> dict[str, Any]:
